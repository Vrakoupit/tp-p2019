...
class appProdUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/subjects')) {
            ...
            // subject_single
            if (preg_match('#^/subjects/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
                    goto not_subject_single;
                }

                return $this->mergeDefaults(array_replace($matches, array('_route' => 'subject_single')), array (  '_controller' => 'AppBundle\\Controller\\SubjectController::showAction',));
            }
            not_subject_single:
